...
class MasterViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        let url = NSURL(string: "http://ws.audioscrobbler.com/2.0/?method=geo.gettoptracks&country=united+states&api_key=6c3349db64b09c651974fce0d2bf11a0&format=json")
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "showDetail" {
            if let indexPath = self.tableView.indexPathForSelectedRow() {
                ...
                //(segue.destinationViewController as! CustomWebViewController).detailItem = object
                
                var artist = object.objectForKey("artist") as? NSDictionary
